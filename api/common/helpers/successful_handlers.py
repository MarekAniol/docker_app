...
def db_commit_success(http_method_type, entity_type=None):
    ...
    def success_post():
         return jsonify({"error": False, "message": SUCCESSFUL_ADDED_MESSAGE.format(entity_type)}), 201
    def success_delete():
        return jsonify({"error": False, "message": SUCCESSFUL_DELETE_MESSAGE.format(entity_type)}), 200
    def success_put():
        return jsonify({"error": False, "message": SUCCESSFUL_UPDATE_MESSAGE.format(entity_type)}), 200
    def default():
        return jsonify({"error": False, "message": SUCCESS_MESSAGE.format(entity_type)}), 200
